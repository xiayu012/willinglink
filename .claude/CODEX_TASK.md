...
## 当前任务（2026-09-13）：把「全屋共同规则协商」接入协商状态机（仅这一条具体规则）

你是本任务唯一实现者。先读 `AGENTS.md`、`CLAUDE.md`、`.claude/PROJECT_STATE.md`、
`.claude/DEVELOPMENT_JUDGMENT.md`，再读 `lib/coordination/README.md`、`types.ts`、`machine.ts`、
`runtime.ts`、`lib/chat/coliving/coordination-session.ts`、`turn.ts` 里的 coordination replace
分支，以及当前 `blacklist.ts` / `features.ts`。

### 老板原始目标

目前协商状态机只处理时间排班。老板要验证它也能协调一条**全屋共同规则**：
**「每个人洗完澡后清理地漏头发」**。这不是住户单方面叫 AI 命令某一位室友整改卫生；
是所有住户一起确认的一条未来共同生活规则。因此不能被现有精确黑名单
「单方面叫别人在洗完澡后清理地漏头发」挡住。

### 最小实现目标

1. 不要把时间/时段字段硬塞进现有排班状态机，也不要大改它。新增一个小而独立、事件溯源的
   **共同规则协商状态机**（名称可清楚自定），它只负责“提出同一条规则 → 向尚未表态的人征询 →
   记录同意/不同意 → 全员同意才定案 → 向全员宣布”的生命周期。它不决定规则内容是否合理，
   不替住户编造规则，不做任何现场卫生程度判断。
2. 只为本次这条具体的“洗完澡后清理地漏头发”的**共同规则协商**接入可测试运行路径。
   可以用小的、明确的自然语言识别/解析入口；不得把“卫生”“地漏”“头发”扩大成一个类别，
   不得让普通单方面卫生要求误进来。单方面点名要求别人清理地漏头发必须仍走原黑名单。
3. 规则文本应由发起人提出的共同规则事实导出；状态机只保存/推进事实与表态，不写固定短信
   话术。给每人的询问/定案通知仍由现有语言生成路径按 doctrine 写，自然、简短、说清合理理由，
   不念流程；但确定性状态机必须保证：
   - 没有所有成员同意，绝不能说规则已经生效；
   - 同意过的人不重复征询；
   - 有人拒绝或提出异议，绝不把原规则强行定案；
   - 不能泄露某个住户发起/投诉的来源；
   - 每轮实际出站和用户回复不能假称已完成。
4. 不改数据库 schema、依赖或环境配置，不恢复 critic，不触碰 `public/sw.js`，不向真人发送。
   复用当前默认关闭的 coordination replace / 隔离测试机制或同等安全机制；不要擅自把未经验证的
   新状态机变成默认生产接管。
5. 新增一份**纯虚构、至少三位住户、至少多轮**的评测语料：第一人提出“咱们能不能定一个规则，
   每个人洗完澡后把地漏里的头发清掉”；另外两人分别明确同意；最后须能观察到规则定案与全员通知。
   语料不写虚假的机器 expect；在免费测试中覆盖状态机不变量和“单方面要求阿川清理地漏头发”
   不会进入共同规则状态机。
6. 更新必要的长期文件：功能精确定义不变；补充“共同规则协商”与单方面要求某人整改是两件不同
   事情。给用户看的功能说明只用用户能懂的语言，不出现状态机/路由/白黑名单等内部词。

### 验证

- 免费：相关状态机单测、`pnpm.cmd coliving:quality`、`pnpm.cmd exec tsc --noEmit`、`git diff --check`。
- 不跑付费 `coliving-eval`、不 commit/push；Codex 将独立阅读 diff，再对新场景按需跑一次
  `--judge-off` 生成报告。

完成时说明：状态机事件/状态、如何区分共同规则与单方面整改、哪些路径接线但默认保持关闭、
修改文件和免费验证结果。

---

## 当前纠正（2026-09-13）：功能必须是死代码级精确行为，不得用“一类”扩大

你是本任务唯一实现者。先读 `AGENTS.md`、`CLAUDE.md`、`.claude/PROJECT_STATE.md`、
`.claude/DEVELOPMENT_JUDGMENT.md`、`.claude/CONCRETE_FUNCTIONS.md`、
`docs/USER_FACING_CAPABILITY_TRUTH.md`、`docs/FEATURE_RUNTIME_ARCHITECTURE.md`，以及当前
`features.ts` / `blacklist.ts` / 功能路由与 corpus-035。

### 老板原始纠正

上一版把黑名单命名为「要求某位室友整改卫生」这一类，范围过宽。老板明确要求：**不要“一类”
功能；功能是死代码级明确、准确、很窄的最小产品行为**。老板会接受很长的功能名字，例如本项
直接叫：**「单方面叫别人在洗完澡后清理地漏头发」**。以后白名单与黑名单里的每一项都遵循
这个粒度；不为省事或高效把多个相邻行为包成一个主题类别。

### 本次目标

1. 把当前黑名单项从宽泛的 `hygiene-rectification` / 「卫生整改要求」收窄、改名为
   **「单方面叫别人在洗完澡后清理地漏头发」**。稳定 id 也应表达这个具体动作，不再用
   hygiene 大类命名。同步所有运行时事实、问答、场景、文档和测试引用。
2. 这项功能只表示：住户单方面交办 AI 去要求**点名的一位室友**，清理由其洗澡/使用浴室后
   留在**地漏里的头发**。corpus-035 原句仍应命中，因为它明确把阿川与地漏里的头发归在一起，
   并直接要求 AI 叫阿川清干净；不要修改老板给定的 `turns[].text`。
3. 明确不属于这项功能的相邻行为至少包括：浴室墙面头发、一般打扫卫生、污渍、异味/开窗、
   地漏维修或疏通（未要求清理头发）、共同商量清洁安排、只抱怨或询问看法。这些即使含有
   “浴室/清理/地漏”等词，也不得被本项阻断；走当前默认宽容的完整流程。
4. 不能只把 `routeDescription` 写窄后相信 LLM。为这个黑名单功能加**很小的、功能自身的
   确定性资格复核**：路由选择 `blocked:<id>` 后，代码还要确认原话同时具备该功能的必要信号
   （点名收件人已由前门保证；地漏 + 头发 + 叫/让/请对方清理的交办语义）。复核不通过则
   当作 `none` 落回完整流程。宁可漏判，不得误杀相邻行为。不要建通用动作卡、大 schema、
   新工具或每功能一次 LLM；可以给黑名单条目一个简单 predicate / qualifier。
5. 检查当前两项已优化功能的现行定义与文字：不要扩大其行为，只在长期架构说明里明确
   “每个条目是准确行为，不是主题类别；未来允许名字很长；每项各自拥有资格复核，精确优先于
   省代码”。本轮不要无证据重写这两项生产逻辑或改变已验收行为。
6. 给住户看的长期文件必须去掉「某一类」「卫生整改」等大类说法，直接列这一个长名字、能做/
   不能做的准确动作和原有真实理由。用户问“刚才为什么”时也必须说这个具体名字，不再说
   「卫生整改要求」。不出现白名单、黑名单、路由、提示词等内部词。
7. 同步 `.claude/PROJECT_STATE.md`、`CLAUDE.md`、`.claude/CONCRETE_FUNCTIONS.md` 与必要架构文档，
   把“功能是精确行为而不是一类主题”的决定写成长期现行原则；历史旧称可保留但须明确已过时。
8. 免费回归必须覆盖：corpus-035 命中；墙面头发、地漏疏通、一般卫生、异味、清洁讨论不命中；
   裸关键词不能绕过完整资格。不要跑付费模型、不要改 DB schema/依赖/env，不发短信、不碰
   `public/sw.js`，不要 commit/push。

完成后运行 `pnpm.cmd coliving:quality`、`pnpm.cmd exec tsc --noEmit`、`git diff --check`，报告
精确功能名/id、资格条件、相邻反例、修改文件和免费结果。

### Codex 独立审查退回：资格正则仍会误把叙述/提问当交办

第一版 qualifier 的 `CLEAN_SIGNAL = /清|收拾|捡|弄|扫|洗|处理|除/` 太宽：功能名字本身
出现「洗完澡」就会提供“洗”信号；「谁弄的不知道」会提供“弄”信号。`ASK_SIGNAL` 中单字
「请」也会把「请问地漏头发怎么清理？」误当成交办。不能依赖前面的 LLM 路由永远不选错，
因为 qualifier 的目的正是对模型选择做确定性复核。

请收窄为**明确的清走动作短语**，并尽量要求清走动作与「地漏里的头发」在同一个局部语义片段
中；不要用 `洗`、`弄`、裸 `清`、裸 `除` 这类可由其它意思满足的单字。征询排除补上
「请问 / 怎么清理 / 如何清理」等自然问法。至少新增以下负例并确保 qualifier=false：

- 「通知阿川，洗完澡后地漏里有头发。」（只是告知存在，没有要求清走）
- 「让阿川知道，地漏里有头发，谁弄的还不知道。」（让=告知对象；弄=来源叙述）
- 「请问地漏里的头发怎么清理？」（问题，不是叫一位室友去办）

仍须保持 corpus-035 原句命中，并覆盖自然但明确的同义动作如「把地漏里的头发清理掉 / 捡走 /
拿走」。只修这个前提错误与对应免费检查；不要扩展架构、不要跑付费模型、不要提交推送。

---

## 当前纠正（2026-09-13）：地漏头发属于已登记黑名单，不是空表

你是本任务唯一实现者。先读 `AGENTS.md`、`CLAUDE.md`、`.claude/PROJECT_STATE.md`、
`.claude/DEVELOPMENT_JUDGMENT.md`、`docs/USER_FACING_CAPABILITY_TRUTH.md`、
`lib/chat/coliving/blacklist.ts`，并在改动前阅读相关的功能前门和路由实现。

### 老板原始决定

老板刚纠正：我们把“目前黑名单为空”写错了。此前反复讨论、并在 corpus-035 中出现的
“请叫阿川把地漏的头发清干净”属于**已明确不让系统执行的黑名单功能**，不是“尚未开发但
仍可交给完整大脑”的普通事项。

理由必须如实保留：系统看不到地漏头发/卫生现场的严重程度；即使用户描述了现场，当前技术
也还没开发到能可靠判断“脏到什么程度才适合替一方要求另一方整改”的水平。因此不能替用户
发出卫生整改要求。它不是因为问题不重要，也不是让用户自己去处理。

### 目标与边界

1. 在 `BLACKLISTED_CAPABILITIES` 登记一个**明确、住户可理解**的功能（建议为“卫生整改要求”），
   覆盖“要求点名室友清理地漏/头发/浴室等卫生残留、整改卫生”的交办；让现有“同一次功能路由
   选出 `blocked:<id>` 才阻断”的机制真正生效。不要退回关键词直接阻断，也不要另加 LLM 调用。
2. **严格限于卫生整改要求**，不要把“说环境有异味”“请求开窗”“讨论清洁安排”“普通提醒”、
   其它黑名单外协调请求一并扩大成禁止。请在任务完成前给出一个正常应拦、一个只差一个条件就
   不应拦的相邻反例，并据此收窄路由说明/测试。
3. 更新给住户看的 `docs/USER_FACING_CAPABILITY_TRUTH.md`：不要再说清单为空。清楚写明这项
   “暂不提供”的功能及上面的真实理由；不出现白名单、黑名单、路由、模型、提示词等内部术语。
   解释应是人能看懂的，不推用户去自己联系、不承诺稍后替他办。
4. 同步 `CLAUDE.md`、`.claude/PROJECT_STATE.md` 和必要的内部架构说明，撤销所有“黑名单目前为空”
   的现行表述。历史记录可保留但须标出已过时。
5. 更新 corpus-035 或新增窄的免费回归，证明地漏头发的交办不会进入完整主生成/`contactPerson`，
   而是被这一个黑名单条目收口；同时证明非卫生整改的正常请求不会被关键词误拦。不要为原句写
   关键词硬编码，不改用户给定的 `turns[].text`。
6. 不改数据库 schema、依赖、环境配置，不碰 `public/sw.js`；不跑付费模型、不发送真实短信，
   不 commit/push。

完成后运行免费三闸：`pnpm.cmd coliving:quality`、`pnpm.cmd exec tsc --noEmit`、`git diff --check`。
报告：登记的功能内容、用户可见说明、拦截/不拦的相邻反例、修改文件和验证结果。

### Codex 实跑退回：黑名单后的“刚才为什么”必须说得上上一件事

本任务第一版经 `corpus-035-unsupported-feature-followup-2026-09-12` 实跑后，第一轮正确：
零工具、零出站，回复说明「卫生整改要求」与看不到程度的理由。但第二轮原话
「请问为什么连这么简单的功能都没有?那你有什么功能？」输出只泛泛列出两项专门优化功能，
遗漏“刚才”指向的卫生整改理由。这不合格：用户明确是在追问上一条被拒绝的事。

请做**最小、低复杂度**修复：黑名单纯代码回复后，保存或复用一条窄的、结构化的“本住户刚刚
被拒绝的黑名单功能 id”事实；紧接着的功能边界问答可据此选择那一个条目，因而必须说出名称与
登记理由。不得读/解析自由文本，不得把其他住户的状态混入，不得通过关键词猜“刚才”，不新增
模型调用、工具、数据库 schema 或大状态机。若用户后来自己换了话题，不应把旧拒绝硬带回来。

更新 corpus-035 的第二轮期望与免费检查，覆盖：同一住户紧接追问能说出卫生整改理由；非紧接、
或别的住户，不能错误继承该引用。不要跑付费模型、不要提交或推送；完成后重跑免费三闸。
