...
## 当前返工（2026-09-13）：两项提醒的短信预览与确认

老板要求去掉面向住户的固定命令模板，同时保留逐项批准的功能边界，避免改动整套架构。
本轮任务与复审反馈已通过 Claude Code CLI 下派；Claude 为唯一实现者，Codex 独立验收后提交。
工程规格见 `docs/reminder-proposal-confirmation.md`，对外边界见 `docs/USER_FACING_CAPABILITY_TRUTH.md`。
验收重点：自然表达可生成明确对象与正文的预览；确认前零第三方出站；取消、过期、跨会话、
重复确认、入队后失败不得导致误发。继续沿用两项功能的固定正文与现有数据表，不新增通用发送权。
独立验证记录：147 项离线检查通过；034/035 隔离重演共八轮结构通过、零模型生成。
报告分别为 `2026-09-13T00-43-40-773Z.html` 与 `2026-09-13T00-43-11-546Z.html`。
tsc 仅剩原有 speech-input.tsx 两处声明冲突。未发送真人短信，未运行语义 judge。
