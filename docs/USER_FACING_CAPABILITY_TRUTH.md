...
> 现在开放的两项功能与仍然做不到的事。
>
> **2026-09-12 补充**：两项功能各自还有一个「近似请求」的**本地指引入口**（见下文），
> 只回当前说话人一句「没有发送 + 唯一命令模板」，零第三方出站、零模型调用；它不是
> 自由文本执行，也没有扩大未开放能力。
...
- 识别：**纯正则，不过模型**；命令体夹带任何多余内容（理由、物品名、别的诉求）一律不认，
  落回普通对话或回一句短指引，**不发送**。住户没按命令写、但明确让 AI 提醒某位指定室友的
  自然语言近似请求，见下面「近似请求的本地指引入口」一节。
...
- 近似请求：住户**明确让 AI 去提醒某位被点名的室友**、主题正好是「用我的个人物品前先问我」，
  但没按固定命令写时，走确定性的本地指引入口（`reminder-ask.ts`）：只回当前说话人一句如实
  说明**没有发送**、并给出唯一命令模板的短指引；**不过模型、零第三方出站**。它不抓纯抱怨、
  评理、讨论/征询、未点名对象（「有人」「他」）、以及混合或其它未开放主题（卫生/头发、费用、
  规则、一般噪音等），这些一律落回普通对话。
...
- 校验全过才发：同一栋房子、名册里唯一、姓名已确认、非本人、当前渠道有地址。
- 回给发起人一句短收据。**整条链路不调用 LLM。**
- 近似请求：住户**明确让 AI 去提醒某位被点名的室友**、主题正好是深夜洗衣，但没按固定命令
  写时，同样走确定性的本地指引入口（`reminder-ask.ts`）：只回当前说话人一句如实说明
  **没有发送**、并给出唯一命令模板的短指引；**不过模型、零第三方出站**。它不抓纯抱怨、评理、
  讨论/征询、未点名对象（「有人」「他」）、以及混合或其它未开放主题（一般噪音、卫生/头发、
  费用、规则等），这些一律落回普通对话。
- 只处理「深夜运行洗衣机 / 烘干机影响别人休息」这一件共同影响，**不要求 AI 先裁定
  谁的卫生阈值或物品所有权**；不能从它外推到一般噪音、一般清洁、规则制定、费用或去留协调。

## 两项功能都有「近似请求」的本地指引入口（不构成自由文本执行）

- 住户没按固定命令写、但**明确让 AI 去提醒某位指定室友**、且主题正好是这两件事之一
  （「用我的个人物品前先问我」/「深夜别开洗衣机或烘干机」）时，走一个**确定性**入口：
  只回当前说话人一句短指引，如实说明**没有发送**，并给出该功能唯一一种固定说法。
- 零第三方出站、零模型调用。它**不是**自由文本执行，也没有扩大任何能力：判定纯字符串，
  只在「显式请求 AI 去提醒 + 名册里确有被点名的室友 + 该功能独有主题 + 不是合规命令」
  同时成立时才命中。
- 纯抱怨、评理、讨论/征询、未点名对象（「有人」「他」）、一般噪音、卫生/头发、费用、
  规则、混合议题等**一律不命中**，仍走现有普通对话——宁可靠漏掉一些自然表达，也不吞掉
  普通谈话。
- 真正发给别人的第三方消息仍然只有两项功能的**写死正文**；近似入口只产生给当前说话人的
  本地指引，不产生任何发给别人的消息。
...
- 近似请求判定核心（两项共用、纯函数）：`lib/chat/coliving/reminder-ask.ts`；
  各模块导出 `hasXAskSignal` / `looksLikeApproximateXAsk` 与
  `PERSONAL_ITEM_APPROXIMATE_GUIDANCE` / `NIGHT_LAUNDRY_APPROXIMATE_GUIDANCE`
