...
export const colivingBrain: Brain = {
  ...
  routes: [
    ...
    //
    // **注意**：已批准功能（个人物品使用提醒 / 夜间洗衣提醒）不靠这条路由——
    // 它们在 `turn.ts` 里由功能前门直接办完、根本不进主生成（见 `features.ts`）。
    // 下面这条只服务**普通对话**里的一对一交办措辞。
    {
      match: [
        /帮我[^。！？，,；;\n]{0,12}(?:问问|问一下|问|说说|说一下|说一声|说下|说|讲|告诉|转告|提|带个?话|捎个?话|把)/,
        /替我[^。！？，,；;\n]{0,12}(?:问问|问|说|讲|告诉|转告|带个?话|捎个?话)/,
        // 直接吩咐 AI「跟/和某位住户说/问」的口语说法，常不带「帮我」，收信人
        // 也常是姓名而不是「他」——「你跟他说一声」「你现在单独跟小浩说」。
        // 这里只判断**这是一次一对一交办**，不解析收信人是谁（收信人由大脑从
        // 原始交办里取、由工具定）；排除明显是对全屋/一群人的说法。
        // `(?!了|过)` 是**祈使/委托收窄**：只认还没做、要 AI 去做的交办；把
        // 「你昨天跟小明说了什么？」「你之前跟房东讲过吗？」这类**问 AI 历史**的
        // 完成式/经验式说法排除掉——它们没有交办任何动作。
        /(?:麻烦你|请你|你能不能|你可以|你)[^。！？，,；;\n]{0,8}(?:跟|和)(?!(?:大家|各位|全屋|所有人|你们|我们))[^。！？，,；;\n]{0,8}(?:说|讲|问|提|转告|提醒|催)(?!了|过)/,
