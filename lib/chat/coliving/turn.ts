...
export async function runColivingTurn(args: {
   ...
   * 有关**的事实（含问能力时那份**索引**：已批准功能按本轮语言取成的名字，由模型自己
   * 决定提哪几条、提几条）；模型只负责说人话，写不出 / 越界就用**同样只含事实源事实**的
   * 兜底。**口径不再是"只两项功能"**：已批准功能只是专门优化的快路径，别的协调请求走
   * 完整协调流程，不是不能做；真正办不了的只有黑名单（当前一项是「单方面叫别人在洗完澡后
   * 清理地漏头发」）。
   *
  ...
  if (isFeatureQaQuestion(args.text)) {
    ...
    const qa = await runFeatureQa({
      text: args.text,
      // 直接给登记好的功能（`label` + `labelEn` 并排登记在功能模块里）：事实源按本轮语言
      // 取成索引交给模型（英文问句因此看到英文功能名，中文口径一字不动），提哪几条由
      // 模型按住户的问题判断——索引是**举例**，不是能力上限。
