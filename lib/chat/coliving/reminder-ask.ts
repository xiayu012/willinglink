 ...
 *
 * **2026-09-13 老板口径：白名单是功能 / 动作边界，不是每一次命中都要二次确认。**
 * 命中后由各功能直接**复用自己既有的受约束执行器**发出固定正文 + 短回执（不过
 * 模型、不再要住户回「确认」）；本文件只负责「这句话是不是该功能的一次明确交办」
 * 这个纯字符串判断，**不做任何发送**。
 *
 ...
 * 不是「对方提醒我」/「我自己去说」。命中即由该功能直接走受约束执行器发送
 * （收件人是否唯一、是否可达由各功能模块校验）。
 */
